...
class PyiCloudSession(requests.Session):
    ...
    def request(self, *args, **kwargs):
        try:
            ...
            logger.debug("..........................")
            log_msg = ("PyiCloud request, args={}, kwargs={}").format(*args, **kwargs)
            logger.debug(log_msg)
    ...
    def _raise_error(self, code, reason):
        try:
            ...
            if code == "ACCESS_DENIED":
                reason = (
                    reason + ".  Please wait a few minutes then try "
...
class PyiCloudService(object):
    ...
    def __init__(
        ...
        try:
            ...
            self.user_agent = (
                "Mozilla/5.0 (iPad; CPU OS 9_3_4 like Mac OS X)"
    ...
    def authenticate(self):
        ...
        if self.session_token is None:
            logger.info(("Error logging into iCloud account {}").format(self.apple_id))
            ...
            if self.session_token is None:
                logger.error(
                    ("Error logging into iCloud account {}").format(self.apple_id)
                )
        ...
        try:
            ...
            req = self.session.post(
                self._setup_endpoint + "/accountLogin", data=json.dumps(data)
            )
    ...
    def get_session_token(self):
        self.clientID = self.generateClientID()
        status, self.appleWidgetKey = self.setupiCloud.requestAppleWidgetKey(
            self.clientID
        )
        ...
        if status:
            session_token, account_country = self.idmsaApple.requestAppleSessionToken(
                self.user["apple_id"], self.user["password"], self.appleWidgetKey
            )
        else:
    ...
    def friends(self):
        try:
            ...
            logger.error(
                (
                    "Find-my-Friends data error. No contacts or "
...
class SetupiCloudService(HTTPService):
    ...
    def requestAppleWidgetKey(self, clientID):
        ...
        self.response = self.session.get(self.urlKey, params=apple_widget_params)
        ...
        try:
            ...
            if error_msg != "" and error_msg != "Missing X-APPLE-WEBAUTH-TOKEN cookie":
    ...
    def requestCookies(self, appleSessionToken, clientID):
        ...
        self.response = self.session.post(
            self.urlLogin, login_payload, params=login_params
        )
        try:
            self.cookies = self.response.headers["Set-Cookie"]
        except Exception as e:
            raise Exception(
                "requestCookies: Cookies query failed", self.urlLogin, repr(e)
            )
        try:
            self.dsid = self.response.json()["dsInfo"]["dsid"]
        except Exception as e:
            raise Exception("requestCookies: dsid query failed", self.urlLogin, repr(e))
...
class IdmsaAppleService(HTTPService):
    ...
    def requestAppleSessionToken(self, user, password, appleWidgetKey):
        ...
        self.response = self.session.post(
            self.urlAuth + appleWidgetKey, user_pw_payload
        )
        try:
            ...
            err_str = "requestAppleSessionToken: " + "Apple Session Token query failed"

            raise Exception(err_str, self.urlAuth, repr(e))
    ...
    def requestApple2svToken(self, appleWidgetKey, user_pw_payload):
        ...
        try:
            ...
            err_str = (
                "requestAppleTwoSVToken: " + "Apple Session 2SV Token query failed"
            )
...
class FindFriendsService(object):
    ...
    def refresh_data(self):
        ...
        req = self.session.post(
            self._friend_endpoint, data=requested_data, params=params
        )
...
class PyiCloudFmFNoDataError(PyiCloudException):
    def __init__(self):
        message = (
            "FmF Data Error. No contacts have been entered in the "
