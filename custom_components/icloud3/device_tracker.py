...
STATIONARY_ZONE_HOME_OFFSET = (
    0.00492  # Subtrace/add from home zone latitude to make stat zone location
)
...
IOS_TRIGGERS_ENTER_ZONE = ["Geographic Region Entered", "iBeacon Region Entered"]
...
# --------------------------------------------------------------------
def TRACE(variable_name, variable1="+++", variable2="", variable3="", variable4=""):
    """
    ...
    if variable_name != "":
        if variable1 == "+++":
            ...
            value_str = "►►►► {} = {} {} {} {}".format(
                variable_name, variable1, variable2, variable3, variable4
            )
...
def setup_scanner(hass, config: dict, see, discovery_info=None):
    ...
    log_msg = (
        "Setting up iCloud3 v{} device tracker for User: {}, " "Group: {}"
    ).format(VERSION, username, group)
    ...
    if waze_region not in WAZE_REGIONS:
        log_msg = (
            "Invalid Waze Region ({}). Valid Values are: "
    ...
    hass.services.register(
        DOMAIN, "icloud3_update", service_callback_update_icloud, schema=SERVICE_SCHEMA
    )
    ...
    hass.services.register(
        DOMAIN,
        "icloud3_restart",
        service_callback_restart_icloud,
    ...
    def service_callback_lost_iphone(call):
        ...
        for group in groups:
            if group in ICLOUD3_GROUP_OBJS:
                ICLOUD3_GROUP_OBJS[group].service_handler_lost_iphone(group, devicename)

    hass.services.register(
        DOMAIN,
        "icloud3_lost_iphone",
        service_callback_lost_iphone,
        schema=SERVICE_SCHEMA,
    )
...
class Icloud(DeviceScanner):
    ...
    def restart_icloud(self):
        ...
        try:
            ...
            event_msg = (
                f"Initializing v{VERSION} > Stage 2, Verify iCloud Location Service"
            )
            self._save_event_halog_info("*", event_msg)
        ...
        try:
            ...
            for devicename in self.devicename_verified:
                ...
                if self.fmf_devicename_email.get(devicename):
                    event_msg = "{} via {}".format(
                        event_msg, self.fmf_devicename_email.get(devicename)
                    )
                ...
                if error_log_msg:
                    ...
                    if self.iosapp_version.get(devicename) == 1:
                        ...
                        event_msg = (
                            "IOS App v2 monitoring > device_tracker.{}, " "sensor.{}"
                        ).format(
                            self.devicename_iosapp.get(devicename),
            ...
            if self.track_devicename_list == "":
                event_msg = (
                    "iCloud3 Error for {} ({}) > No devices to track. " "Setup aborted."
            ...
            event_msg = ("Tracking Devices > {}").format(self.track_devicename_list)
            self._save_event_halog_info("*", event_msg)
            ...
            for devicename in self.tracked_devices:
                ...
                event_msg = (f"Initializing v{VERSION} > Stage 4, " "Set up {}").format(
                    devicename
                )
                self._save_event_halog_info(devicename, event_msg)
                ...
                self._update_stationary_zone(
                    devicename, latitude, longitude, STATIONARY_ZONE_HIDDEN
                )
                ...
                self._update_device_attributes(
                    devicename, kwargs, attrs, "restart_icloud"
                )
    ...
    def _polling_loop_5_sec_device(self, now):
        ...
        try:
            ...
            for devicename in self.tracked_devices:
                ...
                dev_attrs_avail = {
                    k: v for k, v in dev_attrs.items() if k in DEVICE_ATTRS_BASE
                }
                ...
                if self.iosapp_version.get(devicename) == 2:
                    ...
                    if ATTR_LATITUDE not in v2_dev_attrs:
                        ...
                        event_msg = (
                            "iCloud3 Error: IOS App v2 Entity {} does not "
                    ...
                    (
                        v2_state_changed_time,
                        v2_state_changed_secs,
                    ) = self._get_entity_last_changed_time(entity_id)

                    (
                        v2_trigger,
                        v2_trigger_changed_time,
                        v2_trigger_changed_secs,
                    ...
                    ios_update_reason = ""
                    if (
                        v2_state != self.last_v2_state.get(devicename)
                        and v2_state_changed_secs > dev_timestamp_secs
                    ):
                    ...
                    # elif v2_trigger.find('@') >= 0:
                        ...
                        ios_update_reason = "disarded-already processed `{}`{}".format(
                            v2_trigger, v2_trigger.find("@")
                        )
                        pass
                        ...
                        v2_trigger_changed_secs
                        > self.last_v2_trigger_changed_secs.get(devicename)
                    ):
                        update_via_v2_flag = True
                    ...
                    if update_via_v2_flag and debug_msg != self.last_debug_msg.get(
                        devicename
                    ):
                    ...
                    if update_via_v2_flag:
                        ...
                        dev_data[ATTR_ALTITUDE] = self._get_attr(
                            v2_dev_attrs, ATTR_ALTITUDE, NUMERIC
                        )
                        dev_data[ATTR_VERTICAL_ACCURACY] = self._get_attr(
                            v2_dev_attrs, ATTR_VERTICAL_ACCURACY, NUMERIC
                        )
                ...
                if (
                    self.CURRENT_TRK_METHOD_IOSAPP
                ...
                # if lots of accuracy errors. wait and process on 15-second icloud poll
                    ...
                    self.location_isold_cnt.get(devicename) > 5
                    or self.poor_gps_accuracy_cnt.get(devicename) > 5
                ):
                ...
                # Update the device if it wasn't completed last time.
                    ...
                    update_reason = (
                        "iCloud3 Restart not completed, " "retrying location update"
                    )
                ...
                # up here anyway.
                    ...
                    update_reason = "Trigger Change detected > {}@{}".format(
                        dev_trigger, self._secs_to_time(v2_trigger_changed_secs)
                    )
                    self._save_event(devicename, update_reason)
                ...
                if update_via_iosapp_flag:
                    ...
                    dist_from_zone_m = self._current_zone_distance_m(
                        devicename, current_zone, dev_latitude, dev_longitude
                    )
                    ...
                    current_zone_radius_km = self.zone_radius_km.get(
                        current_zone, self.zone_radius_km.get(HOME)
                    )
                    current_zone_radius_m = self.zone_radius_m.get(
                        current_zone, self.zone_radius_m.get(HOME)
                    )
                    ...
                    if dev_trigger in IOS_TRIGGERS_ENTER_ZONE:
                        ...
                        if (
                            current_zone in self.zone_lat
                            and dist_from_zone_m
                            > self.zone_radius_m.get(current_zone) * 2
                            and dist_from_zone_m < 999999
                        ):
                            event_msg = (
                                "Conflicting enter zone trigger, "
                                "Moving into zone > "
                    ...
                    # Push, Manual, Initial
                        ...
                        old_location_flag = self._check_location_isold(
                            devicename, False, dev_timestamp_secs
                        )
                        ...
                        if old_location_flag:
                            ...
                            event_msg = (
                                "Discarding > Old location, Located-{} ({} ago, "
                                "GPS-({}, {}), GPSAccuracy-{}"
                            ).format(
                                self._secs_to_time(dev_timestamp_secs),
                        ...
                        # other things
                            ...
                            if (
                                dist_from_zone_m >= current_zone_radius_m * 2
                                and instr(current_zone, STATIONARY)
                            ...
                            # discard if between 0-zone _radius*4 due to gps errors
                                ...
                                event_msg = (
                                    "Discarding, No Exit Zone trigger, "
                                    "Keeping in zone > Zone-{}, Dist-{}m, "
                                    "DiscardInsideRadius-{}m"
                                ).format(
                            ...
                            # update via icloud to verify location if less than home_radius*100
                                ...
                                dist_from_zone_m <= current_zone_radius_km * 100
                                and self.CURRENT_TRK_METHOD_FMF_FAMSHR
                            ):
                                event_msg = (
                                    "iCloud being called to verify location > "
                                    "Zone-{}, Dist-{}m, ZoneVerifyDist-{}m, "
                                    "GPS-({}, {})"
                                ).format(
                                    current_zone,
                    ...
                    if (
                        dev_data[ATTR_LATITUDE] == None
                        or dev_data[ATTR_LONGITUDE] == None
                    ):
                        update_via_iosapp_flag = False
                        update_via_icloud_flag = True
                ...
                if update_via_iosapp_flag:
                    ...
                    self.iosapp_version.get(devicename) == 2
                    and update_via_v2_flag
                    and ios_update_reason
                ):
                    event_msg = "Discarding > Update already completed `{}`".format(
                        ios_update_reason
                    )
                    self._save_event(devicename, event_msg)
    ...
    def _update_device_iosapp_trigger(self, update_reason, devicename, dev_data):
        ...
        try:
            ...
            event_msg = "IOS App v{} update started, GPS-({}, {})".format(
                self.iosapp_version.get(devicename), latitude, longitude
            )
            self._save_event(devicename, event_msg)
            ...
            self._log_start_finish_update_banner(
                "▼▼▼", devicename, iosapp_version_text, update_reason
            )
            ...
            vertical_accuracy = self._get_attr(
                dev_data, ATTR_VERTICAL_ACCURACY, NUMERIC
            )
            ...
            try:
                if self.device_being_updated_flag.get(devicename):
                    ...
                    info_msg = "Updating"
                info_msg = "● {} {} ●".format(
                    info_msg, self.friendly_name.get(devicename)
                )
            ...
            try:
                for zone in self.track_from_zone.get(devicename):
                    ...
                    self.base_zone = zone
                    self._log_start_finish_update_banner(
                        "▼-▼", devicename, iosapp_version_text, zone
                    )
            ...
            try:
                ...
                if attrs == {}:
                    ...
                    event_msg = (
                        "IOS update was not completed, " "will retry with {}"
                    ).format(self.trk_method_short_name)
                    self._save_event_halog_info(devicename, event_msg)
                ...
                log_msg = ("►LOCATION ATTRIBUTES, State={}, Attrs={}").format(
                    self.state_last_poll.get(devicename), attrs
                )
                self.log_debug_msg(devicename, log_msg)

                self._update_last_latitude_longitude(
                    devicename, latitude, longitude, 1844
                )
    ...
    def _polling_loop_15_sec_icloud(self, now):
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                if current_state != self.state_last_poll.get(devicename):
                    ...
                    update_reason = "State Change Detected for {} > {} to {}".format(
                        devicename, self.state_last_poll.get(devicename), current_state
                    )
                    self._save_event("*", update_reason)

                    log_msg = ("{} {}").format(
                        self._format_fname_devtype(devicename), update_reason
                    )
                    self.log_info_msg(log_msg)
                    ...
                    if self.CURRENT_TRK_METHOD_FMF_FAMSHR:
                        if self.icloud_reauthorizing_account():
                            ...
                            self._display_info_status_msg(
                                devicename, "●●Verify iCloud/FmF Acct●●"
                            )
                            self.info_notification = (
                                "Verification needed for " "account {}"
            ...
            if self.CURRENT_TRK_METHOD_FMF_FAMSHR:
                ...
                self.authenticated_time = dt_util.now().strftime(
    ...
    #     ●►●◄►●▬▲▼◀►►●◀ oPhone=►▶
    #########################################################
    def _update_device_icloud(self, update_reason="CheckiCloud", arg_devicename=None):
        """
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                if dev_data[0] is False:
                    ...
                    if (
                        ...
                        log_msg = (
                            ...
                            self.username, devicename, self.icloud_acct_auth_error_cnt
                        )
                ...
                if latitude == None or longitude == None:
                    ...
                    self._determine_interval_retry_after_error(
                        devicename, self.location_isold_cnt.get(devicename), info_msg
                    )
                ...
                # Discard if poor gps
                    ...
                    info_msg = "Poor.GPS.Accuracy, Dist-{}m (#{})".format(
                        gps_accuracy, self.poor_gps_accuracy_cnt.get(devicename)
                    )

                    self._determine_interval_retry_after_error(
                        devicename, self.poor_gps_accuracy_cnt.get(devicename), info_msg
                    )
                    do_not_update_flag = True
                ...
                if do_not_update_flag:
                    event_msg = (
                        "Discarding > Location-({}, {}), "
                        "GPSAccuracy-{}m, Located-{}, {}"
                    ).format(latitude, longitude, gps_accuracy, timestamp, info_msg)
                    self._save_event(devicename, event_msg)
                ...
                try:
                    ...
                    if not location_isold_flag:
                        self._update_last_latitude_longitude(
                            devicename, latitude, longitude, 4439
                        )
                ...
                try:
                    event_msg = ("{} update completed").format(
                        self.trk_method_short_name
                    )
                    self._save_event(devicename, event_msg)
    ...
    def _get_fmf_data(self, devicename, refresh_fmf_location_data):
        ...
        try:
            if devicename not in self.fmf_location_data:
                log_msg = (
                    "iCloud3 Error: Devicename {} is not in location " "data: {}"
                ).format(devicename, self.fmf_location_data)
                self.log_error_msg
                return ICLOUD_LOCATION_DATA_ERROR
            ...
            if location_data is None:
                if self.icloud_acct_auth_error_cnt > 3:
                    self.log_error_msg(
                        "iCloud3 Error: No FmF Location Data " "Returned for {}"
                    ).format(devicename)
                return ICLOUD_LOCATION_DATA_ERROR
            ...
            try:
                ...
                log_msg = (
                    "►LOCATION DATA, BaseZome={}, TimeStamp={}, "
        ...
        while retry_cnt < 3:
            try:
                ...
                if fmf is None:
                    ...
                    log_msg = ("Reauthenticated FmF account {}").format(self.username)
                    self.log_info_msg(log_msg)
                        ...
                        for location in locations:
                            ...
                            if self.fmf_id.get(contact_id):
                                ...
                                if instr(refresh_msg, devicename) == False:
                                    refresh_msg = "{},{}({})".format(
                                        refresh_msg, devicename, contact_id
                                    )
                        log_msg = ("FmF refresh for {} successful, {}").format(
                            self.username, refresh_msg
                        )
                        self.log_info_msg(log_msg)
    ...
    def _get_famshr_data(self, devicename):
        ...
        try:
            ...
            log_msg = ("Reauthenticated FamShr account {} for {}").format(
                self.username, devicename
            )
            self.log_info_msg(log_msg)
            self._trace_device_attributes(
                devicename, "FamShr Status Reauth", fct_name, status
            )
        ...
        try:
            ...
            device_status = DEVICE_STATUS_CODES.get(
                status[ATTR_ICLOUD_DEVICE_STATUS], "error"
            )
    ...
    def _request_iosapp_location_update(self, devicename):
        ...
        try:
            ...
            if self.iosapp_location_update_secs.get(devicename) > 0:
                ...
                log_msg = "{} {}".format(
                    self._format_fname_devtype(devicename), event_msg
                )
                self.log_debug_msg(devicename, log_msg)
            ...
            attrs[ATTR_INFO] = "● Location update requested (#{}){} ●".format(
                self.count_request_iosapp_update.get(devicename), request_msg_suffix
            )
            self._update_device_sensors(devicename, attrs)

        except Exception as err:
            error_msg = (
                "iCloud3 Error: An error was encountered processing "
    ...
    def _determine_interval(
        ...
        try:
            ...
            location_data = self._get_distance_data(
                devicename, latitude, longitude, gps_accuracy, location_isold_flag
            )
        ...
        try:
            log_msg = ("►DETERMINE INTERVAL Entered, " "location_data={}").format(
                devicename, location_data
            )
            self.log_debug_interval_msg(devicename, log_msg)
            ...
            log_msg = (
                "Zone={} ,IZ={}, NIZ={}, WIZ={}, WNIZ={}, IZH={}, WIZH={}, " "NZ={}"
            ).format(
                current_zone,
                inzone_flag,
                not_inzone_flag,
        ...
        try:
            ...
            #    log_method = '2a-PoorGPSCnt>8'
                ...
                log_method = "4-OldLoc"
                log_msg = "Cnt={}".format(self.location_isold_cnt.get(devicename))
            ...
            if dir_of_travel == STATIONARY:
                ...
                if self.in_stationary_zone_flag.get(devicename) is False:
                    rtn_code = self._update_stationary_zone(
                        devicename, latitude, longitude, STATIONARY_ZONE_VISIBLE
                    )
        ...
        try:
            ...
            log_msg = (
                "►DIR OF TRAVEL ATTRS, Direction={}, LastDir={}, "
            ...
            if (
                ...
                waze_time_msg = self._format_waze_time_msg(
                    devicename, waze_time_from_zone, waze_dist_from_zone_km
                )
        ...
        try:
            ...
            elif instr(current_zone, STATIONARY) or instr(
            ...
            log_msg = (
                "LOCATION TIME-{} loc_timestamp={}, loc_time_secs={}({}), age={}"
            ).format(
                devicename,
                loc_timestamp,
                self._secs_to_time(location_time_secs),
                location_time_secs,
                location_age_str,
            )
            self.log_debug_msg(devicename, log_msg)
            ...
            self.last_distance_str[devicename_zone] = "{} {}".format(
                self._km_to_mi(dist_from_zone_km), self.unit_of_measurement
            )

            self._trace_device_attributes(devicename, "Results", fct_name, attrs)
            results = (
                "BaseZone-{} > Zone-{}, GPS-({}, {}), Interval-{}, "
                "Dist-{} {}, TravTime-{} ({}), NextUpdt-{}, Located-{} ({} ago)"
            ).format(
                self.base_zone,
                attrs[ATTR_ZONE],
                latitude,
            ...
            log_msg = ("{} Device Tracking Complete, Results-{}").format(
                self._format_fname_devtype(devicename), results
            )
            self.log_info_msg(log_msg)
    ...
    def _determine_interval_retry_after_error(self, devicename, retry_cnt, info_msg):
        ...
        try:
            ...
            log_msg = (
                "►DETERMINE INTERVAL ERROR RETRY, ThisZone={}, "
                "LastZone={}, GPS = ({}, {})"
            ).format(this_zone, last_zone, latitude, longitude)
            self.log_debug_interval_msg(devicename, log_msg)
            log_msg = (
                "►DETERMINE INTERVAL ERROR RETRY, Interval={}, "
                "LastUpdt={}, NextUpdt={}, Info={}"
            ).format(interval_str, last_updt_str, next_updt_str, info_msg)
            self.log_debug_interval_msg(devicename, log_msg)
    ...
    def _get_distance_data(
        self, devicename, latitude, longitude, gps_accuracy, location_isold_flag
    ):
        ...
        try:
            ...
            log_msg = (
                "►LAT-LONG GPS INITIALIZED {}, LastDirOfTrav={}, "
                "LastGPS=({}, {}), ThisGPS=({}, {}, UsingGPS=({}, {}), "
                "GPS.Accur={}, GPS.Threshold={}"
            ).format(
                current_zone,
                last_dir_of_travel,
                last_lat,
        ...
        try:
            ...
            debug_log = (
                "3664 dnZone-{}, wStatus-{}, calc_dist-{}, wManualPauseFlag-{},"
                "wCloseToZoneFlag-{}"
            ).format(
                devicename_zone,
                self.waze_status,
            ...
            if calc_dist_from_zone_km <= 0.05 or current_zone == self.base_zone:
                current_zone = self.base_zone
                calc_dist_from_zone_km = 0
                calc_dist_last_poll_moved_km = 0
                calc_dist_from_zone_moved_km = 0
            ...
            debug_log = (
                "dnZone-{}, wStatus-{}, calc_dist-{}, wManualPauseFlag-{},"
                "wCloseToZoneFlag-{}"
            ).format(
                devicename_zone,
                self.waze_status,
            ...
            if self.waze_status == WAZE_USED:
                try:
                    ...
                    if self.waze_status != WAZE_ERROR:
                        ...
                        debug_log = (
                            "3740 Waze request successful, distance {}{}, "
                            "time {} min"
                        ).format(
        ...
        try:
            ...
            log_msg = (
                ...
                current_zone, distance_method, last_dist_from_zone_km, self.waze_status
            )
            self.log_debug_interval_msg(devicename, log_msg)
            log_msg = (
                "►DISTANCES ...Waze, Dist={}, LastPollMoved={}, "
                "FmHomeMoved={}, Time={}, Status={}"
            ).format(
            ...
            self.log_debug_interval_msg(devicename, log_msg)
            log_msg = (
                "►DISTANCES ...Calc, Dist={}, LastPollMoved={}, " "FmHomeMoved={}"
            ).format(
                calc_dist_from_zone_km,
                calc_dist_last_poll_moved_km,
                calc_dist_from_zone_moved_km,
        ...
        try:
            ...
            if (
                ...
                if reset_stat_zone_flag:
                    ...
                    log_msg = (
                        "►STATIONARY ZONE, Reset timer, " "Moved={}, Timer={}"
                    ).format(
                        calc_dist_last_poll_moved_km,
                ...
                # distance moved and check to see if now in a stationary zone
                    ...
                    log_msg = (
                        "►STATIONARY ZONE, Small movement check, "
                        "TotalMoved={}, Timer={}, TimerLeft={}, CloseTimerLeft={}, "
                        "DistFmZone={}, CloseDist={}"
                    ).format(
                    ...
                    if move_into_stationary_zone_flag:
                        ...
                        dir_of_trav_msg = "Age={}s, Moved={}".format(
                            self._secs_to(self.stat_zone_timer.get(devicename)),
                            self.stat_zone_moved_total.get(devicename),
                        )
                else:
                    self.stat_zone_moved_total[devicename] = 0
            ...
            log_msg = (
                "►GET DEVICE DISTANCE DATA Complete, "
            ...
            log_msg = ("►DISTANCE DATA={}-{}").format(devicename, distance_data)
            self.log_debug_msg(devicename, log_msg)
    ...
    def _update_device_attributes(
        ...
        # to the current zone that was based on the device location.
            ...
            self._is_inzone(state)
            and self._is_inzone(current_zone)
            and state != current_zone
        ):
            event_msg = (
                "State/Zone mismatch > Setting `state` value ({}) "
                "to `zone` value ({})"
            ).format(state, current_zone)
            self._save_event(devicename, event_msg)
            state = current_zone
        ...
        if update_took_time > 3 and ATTR_INFO in attrs:
            attrs[ATTR_INFO] = "{} ●Took {}s".format(attrs[ATTR_INFO], update_took_time)
        ...
        if instr(new_trigger, "@") == False:
            new_trigger = "{}@{}".format(
                new_trigger, self._secs_to_time(self.this_update_secs)
            )
        ...
        try:
            ...
            while retry_cnt < 99:
                ...
                log_msg = ("Verify Check #{}. Expected {}, Read {}").format(
    ...
    def _setup_base_kwargs(
        ...
        if zone_name and self._is_inzoneZ(state):
            ...
            # elif (zone_dist <= self.zone_radius_km.get(zone_name)*1000 and
                ...
                event_msg = (
                    "Moving to zone center > {}, " "GPS-({}, {}) to ({}, {}), Dist-{}m"
                ).format(zone_name, latitude, longitude, zone_lat, zone_long, zone_dist)
                self._save_event(devicename, event_msg)
    ...
    def _format_fname_devtype(self, devicename):
        return "{}({})".format(
            self.friendly_name.get(devicename), self.device_type.get(devicename)
        )
    ...
    def _trace_device_attributes(self, devicename, description, fct_name, attrs):
        ...
        try:
            ...
            log_msg = ("{} Attrs={}{}").format(
                description, trace_attrs, trace_attrs_in_attrs
            )
            self.log_debug_msg(devicename, log_msg)
    ...
    def _get_current_zone(self, devicename, latitude, longitude):
        ...
        for zone in self.zone_lat:
            ...
            smaller_zone = zone_dist == zone_selected_dist and self.zone_radius_km.get(
                zone
            ...
            log_msg += ("{}-{}km/r{}, ").format(
                zone, zone_dist, round(self.zone_radius_m.get(zone))
            )
        ...
        log_msg = (
            "►GET CURRENT ZONE END, Zone={}, GPS=({}, {}), "
            "StateThisPoll={}, LastZone={}, ThisZone={}"
        ).format(
            zone_selected,
            latitude,
            longitude,
    ...
    def _update_last_latitude_longitude(
        self, devicename, latitude, longitude, line_no=0
    ):
        ...
        if latitude == None or longitude == None:
            error_log = (
                "Unusable location > No location available ({}, {}), "
                "Ignored (line {})"
            ).format(latitude, longitude, line_no)
            ...
            error_log = (
                "Unusable location > Located at Stationary Base Zone ({}, {}), "
                "Ignored (line {})"
            ).format(latitude, longitude, line_no)
        else:
    ...
    def _update_stationary_zone(
        self, devicename, arg_latitude, arg_longitude, visible_flag=False
    ):
        ...
        try:
            ...
            zone_dist_msg = (
                f"{zone_dist}m" if zone_dist < 500 else f"{round(zone_dist/1000, 2)}km"
            )
            ...
            event_msg = (
                "Set Stationary Zone > {}, GPS-({}, {}), DistFromLastLoc-{}"
            ).format(zone_name, latitude, longitude, zone_dist_msg)
            self._save_event_halog_info(devicename, event_msg)
        ...
        try:
            ...
            if prefix_zone == HOME:
                ...
                base_entity = "sensor.{}_{}".format(
                    prefix_zone, self.sensor_prefix_name.get(devicename)
                )
    ...
    def _format_info_attr(
        ...
        try:
            ...
            if not self.CURRENT_TRK_METHOD_FMF:
                info_msg = "{} ●Track.Method: {}".format(
                    info_msg, self.trk_method_short_name
                )
            ...
            if gps_accuracy > self.gps_accuracy_threshold:
                ...
                if self.poor_gps_accuracy_cnt.get(devicename) > 0:
                    info_msg = "{} (#{})".format(
                        info_msg, self.poor_gps_accuracy_cnt.get(devicename)
                    )
                if (
                    self._is_inzoneZ(current_zone)
            ...
            if isold_cnt > 0:
                ...
                info_msg = "{} ●Old.Location, Age-{} (#{})".format(
                    info_msg, self._secs_to_time_str(age), isold_cnt
                )
            ...
            if self.waze_data_copied_from.get(devicename) is not None:
                ...
                if devicename != copied_from:
                    info_msg = "{} ●Using Waze data from {}".format(
                        info_msg, self.friendly_name.get(copied_from)
                    )
            ...
            if (poll_cnt > 0 and poll_cnt % 5) == 0:
                info_msg = ("{} ●Metrics: {}").format(
                    info_msg, self._format_usage_counts(devicename)
                )
    ...
    def _format_usage_counts(self, devicename):
        ...
        try:
            usage_msg = (
                "iCloud.Locates-{}, IOS.App.Updates-{}, Discarded-{}, "
                "State.Changes-{}, Trigger.Changes-{}"
            ).format(
                self.count_update_icloud.get(devicename),
                self.count_update_iosapp.get(devicename),
                self.count_update_ignore.get(devicename),
            ...
            if self.count_request_iosapp_update.get(devicename) > 0:
                usage_msg = "{}, IOS.APP.Locates-{}".format(
                    usage_msg, self.count_request_iosapp_update.get(devicename)
                )
        except Exception as err:
            _LOGGER.exception(err)
    ...
    def _define_device_status_fields(self):
        ...
        self.fmf_location_data = {}
        self.iosapp_version = {}
        self.iosapp_v2_last_trigger_entity = (
            {}
    ...
    def _setup_tracking_method(self, tracking_method):
        ...
        if (
            ...
            and PYICLOUD_IC3_IMPORT_SUCCESSFUL is False
        ):
            trk_method_primary = TRK_METHOD_IOSAPP

        self.CURRENT_TRK_METHOD_IOSAPP = trk_method_primary in TRK_METHOD_IOSAPP_IOSAPP1
        self.USE_IOSAPPV1_TRK_METHOD = (
            trk_method_primary == TRK_METHOD_IOSAPP1
            or trk_method_secondary == TRK_METHOD_IOSAPP1
        )
        ...
        for zone in zones:
            ...
            try:
                ...
                if ATTR_LATITUDE in zone_data:
                    ...
                    self.zone_friendly_name[zone_name] = zone_data.get(
            ...
            zone_msg = (
                f"{zone_msg}{zone_name}/{self.zone_friendly_name.get(zone_name)}"
    ...
    def _define_stationary_zone_fields(
        ...
        offset = (
            -1 * offset
            if self.zone_home_lat > 89.8 or self.zone_home_lat < -89.8
            else offset
        )
        self.stat_zone_base_lat = round(self.zone_home_lat + offset, 6)

        log_msg = (
            f"Set up Initial Stationary Zone Location > "
    ...
    def _initialize_device_fields(self, devicename):
        ...
        self.device_tracker_entity[devicename] = ("{}.{}").format(DOMAIN, devicename)
    ...
    def _initialize_waze_fields(
        ...
        if self.distance_method_waze_flag:
            log_msg = (
                "Set Up Waze > Region-{}, MaxDist-{} {}, " "MinDist-{} {}, Realtime-{}"
            ).format(
                self.waze_region,
                waze_max_distance,
                self.unit_of_measurement,
    ...
    def _initialize_pyicloud_device_api(self):
        ...
        if (
            ...
            event_msg = ("iCloud3 Error: Falling back to IOSAPP instead of {}").format(
                self.trk_method_s
            )
            self._save_event_halog_error("*", event_msg)
        ...
        if self.CURRENT_TRK_METHOD_IOSAPP:
            ...
            try:
                ...
                event_msg = (
                    ...
                    "use the IOS App tracking_method instead."
                ).format(self.username, self.group, self.trk_method_short_name)
                self._save_event_halog_error("*", event_msg)
                self.trk_method = TRK_METHOD_IOSAPP
        ...
        try:
            ...
            if fmf == None:
                log_msg = (
                    ...
                    "Acount/Find-my-Friends Tracking Method`."
                ).format(self.username, self.group)
                self._save_event_halog_error("*", log_msg)
                log_msg = (
                    "iCloud3 Error for {} ({}) > "
            ...
            if contacts == []:
                ...
                log_msg = (
                    "FmF Contact information is not available. "
            ...
            for devicename in self.devicename_verified:
                if self.devicename_verified.get(devicename) is False:
                    ...
                    log_msg = (
                        "iCloud3 Error for {} ({}) > " "Valid contact emails are {}"
                    ).format(self.username, self.group, contacts_valid_emails[:-2])
                    self._save_event_halog_error("*", log_msg)
                    log_msg = (
                        "iCloud3 Error for {} ({}) > "
                        "The email address for {} ({}) is invalid or is not "
                        "in the FmF contact list."
                    ).format(self.username, self.group, devicename, parm_email)
                    self._save_event_halog_error("*", log_msg)
    ...
    def _setup_tracked_devices_for_famshr(self):
        ...
        try:
            ...
            for device in self.api.devices:
                ...
                api_devicename_list = "{}, {}".format(api_devicename_list, devicename)
            ...
            event_msg = ("iCloud Account devices{}").format(api_devicename_list)
        ...
        try:
            for track_device_line in config_parameter:
                ...
                if self._check_devicename_in_another_thread(devicename):
                    ...
                    self._save_event_halog_error("*", event_msg)
                    event_msg = (
                        "iCloud3 Error: Then do the following: (1) Select "
                        ...
                        "(6) Restart HA."
                    ).format(devicename, devicename, devicename)
                    self._save_event_halog_error("*", event_msg)
                    event_msg = (
                        "iCloud3 Error: Conflicting device_tracker names `{}`. "
                ...
                if di[DI_SENSOR_IOSAPP_TRIGGER]:
                    self.iosapp_v2_last_trigger_entity[devicename] = di[
                        DI_SENSOR_IOSAPP_TRIGGER
                    ]
                if di[DI_ZONES]:
    ...
    # --------------------------------------------------------------------
    def _decode_track_device_config_parms(self, track_device_line, iosapp_v2_entities):
        """
        ...
        try:
            ...
            if scan_entity_registry:
                ...
                log_msg = (
                    "Scanning {} for entity registry for IOS App v2 "
                    "device_tracker for {}"
                ).format(self.entity_registry_file, devicename_iosappv2_id)
                if iosappv2_id != "":
                    log_msg += (", devicename suffix specified ({})").format(
                        iosappv2_id
                    )
                self.log_info_msg(log_msg)
                ...
                for entity in (
                    x
                    for x in iosapp_v2_entities
                    if x["entity_id"].startswith("device_tracker.")
                ):
                    v2er_devicename = entity["entity_id"].replace(
                        "device_tracker.", "", 5
                    )
                    ...
                    if v2er_devicename.startswith(devicename) or devicename.startswith(
                        v2er_devicename
                    ):
                        log_msg = (
                            "Matched IOS App v2 entity {} with "
                            "iCloud3 tracked_device {}"
                        ).format(v2er_devicename, devicename)
                        self.log_info_msg(log_msg)
                ...
                if device_id:
                    for entity in (
                        ...
                        if entity["device_id"] == device_id:
                            sensor_last_trigger_entity = entity["entity_id"].replace(
                                "sensor.", "", 5
                            )
                            log_msg = (
                                "Matched IOS App v2  {} with "
                                "iCloud3 tracked_device {}"
                            ).format(entity["entity_id"], devicename)
                            self.log_info_msg(log_msg)
                            break
            ...
            if self.iosapp_version[devicename] == 1:
                if scan_entity_registry:
                    event_msg = (
                        "Determine IOS App version > `device_tracker.{}` "
    ...
    def _setup_sensor_base_attrs(self, devicename):
        ...
        for zone in self.track_from_zone.get(devicename):
            ...
            event_msg = ("Sensor entity prefix > sensor.{}{}").format(
                zone_prefix, self.sensor_prefix_name.get(devicename)
            )
            self._save_event(devicename, event_msg)

        log_msg = (
            "Set up sensor name for device, devicename={}, " "entity_base={}"
        ).format(devicename, self.sensor_prefix_name.get(devicename))
        self.log_debug_msg(devicename, log_msg)
    ...
    #
    #########################################################
    def _check_location_isold(
        self, devicename, arg_location_isold_flag, timestamp_secs
    ):
        """
        ...
        try:
            ...
            log_msg = ("►CHECK ISOLD, Time={}, isOldFlag={}, Age={}").format(
                self._secs_to_time(timestamp_secs), arg_location_isold_flag, age_str
            )
            self.log_debug_msg(devicename, log_msg)
    ...
    def _log_device_status_attrubutes(self, status):
        ...
        log_msg = (
            "►ICLOUD DEVICE STATUS/LOCATION, ●deviceDisplayName={}, "
            "●deviceStatus={}, ●name={}, ●deviceClass={}, "
            "●batteryLevel={}, ●batteryStatus={}, "
            "●isOld={}, ●positionType={}, ●latitude={}, ●longitude={}, "
            "●horizontalAccuracy={}, ●timeStamp={}({})"
        ).format(
    ...
    def _save_event(self, devicename, event_text):
        ...
        try:
            ...
            if devicename == "*" or state is None:
                state = ""
            if devicename == "*" or zone is None:
                zone = ""
            if devicename == "*" or distance is None:
                distance = ""
            if devicename == "*" or travel_time is None or travel_time == 0:
                travel_time = ""
            if devicename == "*" or interval is None:
                interval = ""
            if devicename == "*" or devicename is None:
                devicename = "*"

            if instr(state, STATIONARY):
                state = STATIONARY
            ...
            while et_pos_start >= 0:
                ...
                if et_pos_start < 0:
                    ...
                    event_recd = [devicename, "", "", "", "", "", "", event_text_chunk]
    ...
    def _update_event_log_sensor_line_items(self, devicename):
        ...
        try:
            ...
            attr_recd = {}
            attr_event = {}
            log_attrs["log_level_debug"] = (
                "On" if self.log_level_eventlog_flag else "Off"
            )
    ...
    def _get_waze_data(
        ...
        try:
            ...
            log_msg = (
                "►WAZE DISTANCES CALCULATED>, "
                "Status={}, DistFromHome={}, TimeFromHome={}, "
                " DistLastPoll={}, "
                "WazeFromHome={}, WazeFromLastPoll={}"
            ).format(
                waze_status,
                waze_dist_from_zone_km,
                waze_time_from_zone,
        ...
        try:
            for near_devicename_zone in self.waze_distance_history:
                ...
                if dist_from_other_waze_data < test_distance:
                    event_msg = (
                        "Waze history data used from {} ({}m away), "
                        "Dist from `{}` zone-{}km, Travel time-{}min"
                    ).format(
                        near_devicename_zone,
    ...
    # --------------------------------------------------------------------
    def _format_waze_time_msg(
        self, devicename, waze_time_from_zone, waze_dist_from_zone
    ):
        """
    ...
    def _verify_waze_installation(self):
        ...
        if WAZE_IMPORT_SUCCESSFUL == "YES" and self.distance_method_waze_flag:
            self.waze_status = WAZE_USED
        else:
            self.waze_status = WAZE_NOT_USED
            self.distance_method_waze_flag = False
                ...
                if group != self.group and ICLOUD3_GROUPS.index(group) > 0:
                    ...
                    for tracked_device in tracked_devices:
                        ...
                        if devicename == tracked_devicename:
                            log_msg = (
                                "Error: A device can only be tracked in "
                                "one platform/group {}. '{}' was defined multiple "
                                "groups and will not be tracked in '{}'."
                            ).format(ICLOUD3_GROUPS, devicename, self.group)
                            self._save_event_halog_error("*", log_msg)
                            return True
    ...
    def trace2(
        ...
        if variable_name != "":
            ...
            if variable1 != "%":
                value_str += f" ({variable1})"
            if variable2 != "%":
                value_str += f" ({variable2})"
            if variable3 != "%":
                value_str += f" ({variable3})"
            if variable4 != "%":
                value_str += f" ({variable4})"
    ...
    def _time_to_12hrtime(self, hhmmss, time_24h=False):
        ...
        if self.unit_of_measurement == "mi" and time_24h is False:
            ...
            if hhmmss_hh > 12:
                ...
                hhmmss_hh = 12
            hhmmss = "{}:{}:{}".format(hhmmss_hh, hhmmss_hms[1], hhmmss_hms[2])
        return hhmmss
    ...
    def _timestamp_to_time_utcsecs(self, utc_timestamp):
        ...
        ts_str = dt_util.utc_from_timestamp(ts_local).strftime(self.um_time_strfmt)
        if ts_str[0] == "0":
            ts_str = ts_str[1:]
    ...
    def _timestamp_to_secs(self, timestamp, utc_local=LOCAL_TIME):
        ...
        try:
            ...
            _LOGGER.error("Invalid timestamp format, timestamp = '%s'", timestamp)
            _LOGGER.exception(err)
            tm = 0
    ...
    def icloud_need_trusted_device(self):
        ...
        for i, device in enumerate(devices):
            ...
            self.valid_trusted_device_ids = "{},{}".format(
                i, self.valid_trusted_device_ids
            )

        description_msg = (
            "Enter the ID for the Trusted Device to receive "
            "the verification code via a text message.\n\n\n{}"
        ).format(device_list)

        ICLOUD_ACCT_DEVICE_CONFIG[self.group] = configurator.request_config(
            "iCloud Account Verification Required for {}-{}".format(
                self.username, self.group
            ),
    ...
    def icloud_trusted_device_callback(self, callback_data):
        ...
        self.trusted_device = self.api.trusted_devices[self.trusted_device_id]
        ...
        if str(self.trusted_device_id) not in self.valid_trusted_device_ids:
            event_msg = (
                "iCloud3 Error: Invalid Trusted Device ID. " "Entered={}, Valid IDs={}"
            ).format(self.trusted_device_id, self.valid_trusted_device_ids)
            self._save_event_halog_error(event_msg)
    ...
    def icloud_verification_callback(self, callback_data):
        ...
        try:
            ...
            log_msg = (
                "iCloud3 Error: Failed to verify verification " "code: {}"
            ).format(error)
            self.log_error_msg(log_msg)
    ...
    def service_handler_lost_iphone(self, group, arg_devicename):
        ...
        log_msg = ("iCloud Lost iPhone Alert, Device {}").format(arg_devicename)
        self.log_info_msg(log_msg)
        self._display_status_info_msg(arg_devicename, "Lost Phone Alert sent")

    # --------------------------------------------------------------------
    def service_handler_icloud_update(
        ...
        log_msg = ("iCloud3 Command Entered, Device: {}, ({}), " "Command: {}").format(
            arg_devicename, group, arg_command
        )
        self.log_debug_msg("*", log_msg)
        # self._save_event("*", log_msg)
        ...
        if instr(arg_command, "event_log") == False:
            self._save_event(
                arg_devicename,
                "Service Call Command " "received ({})".format(arg_command),
            )
        ...
        log_msg = ("iCloud3 Command Processed, Device: {} ({}), " "Command: {}").format(
            arg_devicename, group, arg_command
        )
        ...
        if arg_command_cmd == "restart":
            ...
            error_msg = (
                "Error > Then refresh the Event Log page in your browser. v2.1 "
            ...
            error_msg = (
                "Error > Event Log v1.0 is being used. Clear your browser "
        ...
        for devicename in self.tracked_devices:
            ...
            if arg_command_cmd == "pause":
                ...
                cmd_type = CMD_ERROR
                info_msg = "● INVALID COMMAND ({}) ●".format(arg_command_cmd)
                self._display_info_status_msg(devicename, info_msg)
    ...
    # --------------------------------------------------------------------
    def service_handler_icloud_setinterval(
        ...
        if arg_devicename and self.CURRENT_TRK_METHOD_IOSAPP:
            if (
                ...
                event_msg = (
                    "Can not Set Interval, location request cnt " "exceeded ({} of {})"
                ).format(
                    self.count_request_iosapp_update.get(arg_devicename),
                    self.max_iosapp_locate_cnt,
                )
        ...
        for devicename in self.tracked_devices:
            ...
            log_msg = (
                "►SET INTERVAL COMMAND Start {}, "
                "ArgDevname={}, ArgInterval={}, "
                "New Interval: {}"
            ).format(devicename, arg_devicename, arg_interval, new_interval)
            self.log_debug_msg(devicename, log_msg)
            self._save_event(
                devicename,
                ("Set Interval Command handled, " "New interval {}").format(
                    arg_interval
                ),
            )
    ...
    def _service_multi_acct_devicename_check(
        ...
        event_msg = ("{}-{} Processed").format(info_msg, arg_devicename)
        # self._save_event(arg_devicename, event_msg)
        self.log_info_msg(event_msg)
        return True
