...
-rc4 5/30/2020
    ...
    - Error encountered when sending a message to a device are now caught. The error and message are displayed in the HA log and on the Event Log.
...
-rc3 5/27/2020
    - Fixed an issue where the iOS App location request time was not set available, creating an error wwhen restarting ic3 multiple of times on the Event Log screen.
    - iCloud3 was using legacy HA DeviceScanner code that was made available for peeople running HA versions before 0.95. iCloud3 will now use the current code rather than legacy code.
...
#--------------------------------------------------------------------
    def _verify_waze_installation(self):
        ...
        if (WAZE_IMPORT_SUCCESSFUL == 'YES'
                and self.distance_method_waze_flag):
            self.waze_status = WAZE_USED
        else:
            self.waze_status = WAZE_NOT_USED
            self.distance_method_waze_flag = False
