...
#    $$  - DodgerBlue        **  - Fushia
#    $$$ - MediumPink        *** - OrangeRed
EVLOG_COLOR_DEBUG        = "$$$"
EVLOG_COLOR_AUTHENTICATE = ""
EVLOG_COLOR_IMPORTANT    = "***"
...
#--------------------------------------------------------------------
    def _start_icloud3(self):
        ...
        TRACE("start_icloud3_inprocess_flag",self.start_icloud3_inprocess_flag)
        ...
        try:
            ...
            for devicename in self.tracked_devices:
                ...
                if self.iosapp_version.get(devicename) == 2:
                    ...
                    self.last_battery[devicename] = dev_battery
                    if update_via_v2_flag:
                ...
                if (self.TRK_METHOD_IOSAPP
                        and self.zone_current.get(devicename) == ''):
                ...
                if update_method == IOSAPP_UPDATE:
                    ...
                    #elif self._check_in_zone_and_before_next_update(devicename):
                    #    event_msg = (f"Discarding > In zone {self._format_fmane_zone(zone)} "
                    #        f"and Next Update Time not reached "
    ...
    def _update_device_iosapp(self, devicename, update_reason, dev_data):
        ...
        try:
            ...
            if self.next_update_time.get(devicename_zone) == PAUSED:
                return
            elif ATTR_LATITUDE not in dev_data or ATTR_LONGITUDE not in dev_data:
                return
            elif dev_data[ATTR_LATITUDE] == None or dev_data[ATTR_LONGITUDE] == None:
                return
            ...
            event_msg = (f"iOS App update started "
...
#########################################################
    ...
    #       the distance will be 0.
        ...
        try:
            ...
            self._save_event(devicename, event_msg)
            log_msg = f"Location Updated Complete > {self._format_fname_devtype(devicename)}, {event_msg}"
            self.log_info_msg(log_msg)
        ...
        try:
            ...
            if (calc_dist_from_zone_km > self.stat_min_dist_from_zone_km
                ...
                #distance moved and check to see if now in a stationary zone
                    ...
                    elif (self.iosapp_update_flag.get(devicename)
                            and self.state_this_poll.get(devicename) == STATIONARY):
                        move_into_stationary_zone_flag = True
...
#--------------------------------------------------------------------
    def _update_device_attributes(self, devicename, kwargs: str = None,
        ...
        elif ((state == STATIONARY  and self._is_inzone(zone))
                or (self._is_inzone(state)  and self._is_inzone(zone)  and state != zone)):
        ...
        #Get friendly name or capitalize and reformat state, reset waze history
...
#--------------------------------------------------------------------
    def _get_iosappv2_device_sensor_battery_level(self, devicename) -> int:
        return -1

        entity_id = (f"sensor.{self.iosapp_v2_battery_level_entity.get(devicename)}")
...
#########################################################
    def _define_tracking_control_fields(self):
        ...
        self.iosapp_v2_battery_level_entity  = {} #sensor entity extracted from entity_registry
        ...
        try:
            ...
            for device in api_device_content:
                ...
                if devicename in self.devicename_verified:
                    ...
                    self.log_level_debug_rawdata_flag = self.log_level_debug_flag
                    self.log_level_debug_rawdata("FamShr iCloud Data - (device) 5788", device)
                    self.log_level_debug_rawdata_flag = False
        ...
        try:
            for track_device_line in config_parameter:
                ...
                if di[DI_SENSOR_BATTERY_LEVEL]:
                    self.iosapp_v2_battery_level_entity[devicename] = di[DI_SENSOR_BATTERY_LEVEL]
                if di[DI_ZONES]:
        ...
        try:
            ...
            if scan_entity_registry:
                ...
                for entity in (x for x in iosapp_v2_entities \
                    ...
                    if (v2er_devicename.startswith(devicename)
                            or devicename.startswith(v2er_devicename)):
                ...
                if device_id:
                    sensor_last_trigger_entity = \
                        self._get_entity_registry_item(devicename, iosapp_v2_entities, device_id, '_last_update_trigger')
                    sensor_battery_level_entity = ''
                    #    self._get_entity_registry_item(devicename, iosapp_v2_entities, device_id, '_battery_level')
...
#--------------------------------------------------------------------
    def _get_entity_registry_item(self, devicename, iosapp_v2_entities, device_id, desired_entity_name):
        '''
    ...
    def _check_ic3_event_log_file_version(self):
        ...
        try:
            ...
            if ic3_version > www_version:
                ...
                event_msg = (f"{EVLOG_COLOR_IMPORTANT}"
