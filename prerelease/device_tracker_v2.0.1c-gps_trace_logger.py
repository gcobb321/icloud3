...
VERSION = '2.0.1c'     #Custom Component Updater
'''
2.0.1
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                #icloud poll
                    ...
                    event_msg = "_Discarded, Old data or Poor GPS"
                    self._save_event(devicename, event_msg)
                    continue
                ...
                #zone in error. Try to reset to the last zone location.
                    ...
                    if (self.last_lat.get(devicename) == BASE_STATIONARY_LAT or
                        ...
                        log_msg = ("iCloud3 Error: IOS App entered initial "
                            "Stationary Zone. Resetting to previous location "
                            "({}:{}) to ({}:{}). Will try to correct.").format(
                ...
                if update_via_iosapp_flag:
                    ...
                    #Push, Manual, Initial    
                        ...
                        if old_location_flag:
                            ...
                            event_msg = "__Discarded, old location"
                            self._save_event_halog_info(devicename, event_msg)
        ...
        try:
            for devicename in self.tracked_devices:
                ...
                if do_not_update_flag:
                    event_msg = ("__Discarded > Location-({}, {}), "
        ...
        try:
            ...
            if dir_of_travel == STATIONARY:
                ...
                if self.in_stationary_zone_flag.get(devicename) is False:
                    ...
                    if rtn_code:
                        event_msg = ("__Stationary Zone Location Set ({}, {})").format(
                            latitude,
                            longitude)
                        self._save_event(devicename, event_msg)
...
#########################################################
    def _determine_interval_retry_after_error(self, devicename, retry_cnt, info_msg):
        ...
        try:
            ...
            self.trace('3565 devicename',devicename,this_zone,last_zone)
            
            self.trace('_is_inzone',this_zone,self._is_inzone(devicename))
            self.trace('this-zone-lat/long',self.zone_latitude.get(this_zone),self.zone_longitude.get(this_zone))

            self.trace('_was_inzone',last_zone,self._was_inzone(devicename))
            self.trace('last-zone-lat/long',self.zone_latitude.get(last_zone),self.zone_longitude.get(last_zone))
            
            self.trace('last-lat/long',self.last_lat.get(devicename),self.last_long.get(devicename))
            
            if self._is_inzone(devicename):
                self.trace('process if is-inzone')
                latitude  = self.zone_latitude.get(this_zone)
                longitude = self.zone_longitude.get(this_zone)
            elif self._was_inzone(devicename):
                self.trace('process if was-inzone')
                latitude  = self.zone_latitude.get(last_zone)
                longitude = self.zone_longitude.get(last_zone)
            else:
                self.trace('process else')
                latitude  = self.last_lat.get(devicename)
                longitude = self.last_long.get(devicename)

            if latitude == None or longitude == None:
                self.trace('None-Reset to last lat/long')
                latitude  = self.last_lat.get(devicename)
                longitude = self.last_long.get(devicename)
            if latitude == None or longitude == None:
                self.trace('None2-Reset to last zone lat/long')
                latitude  = self.zone_latitude.get(last_zone)
                longitude = self.zone_longitude.get(last_zone) 
            if latitude == None or longitude == None:               
                self.trace('None3-abort update')
...
#--------------------------------------------------------------------
    def _update_last_latitude_longitude(self, devicename, latitude, longitude, line_no=0):
        ...
        if latitude != BASE_STATIONARY_LAT and longitude != BASE_STATIONARY_LONG:
            ...
            self.trace('4668 _update_last_latitude_longitude',line_no)
            self.trace('last-lat/long',self.last_lat.get(devicename),self.last_long.get(devicename))
            return True
        else:
            return False
...
#--------------------------------------------------------------------
    def _update_stationary_zone(self, devicename,
        ...
        try:
            ...
            self.trace('4695 setStatZoneLoc',zone_name)
            self.trace('stat-zone-lat/long',latitude,longitude)
            ...
            event_msg = ("Stationary Zone Created > {}").format(zone_name)
                #self._format_fname_devtype(devicename))
            self._save_event_halog_info(devicename, event_msg)
...
#--------------------------------------------------------------------
    def _initialize_zone_tables(self):
        ...
        for zone in zones:
            ...
            self.trace('5210 iZoneTbl',zone_name)
            self.trace('init-zone-lat/long',zone_data[ATTR_LATITUDE],zone_data[ATTR_LONGITUDE])
...
#--------------------------------------------------------------------
    def _initialize_debug_control(self, log_level):
        ...
        self.log_level_intervalcalc_flag = DEBUG_TRACE_CONTROL_FLAG or (log_level.find('intervalcalc') > -1)
...
#--------------------------------------------------------------------
    def _initialize_device_tracking_fields(self, devicename):
        ...
        self.trace('5328 init_last_latitude_longitude')
        self.trace('last-lat/long',self.last_lat.get(devicename),self.last_long.get(devicename))
